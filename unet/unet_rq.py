...
from rq_function import RieszQuincunx
...
class DownConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, alpha, pooling=True, batchnorm=True, dropout=False, shrink=True):
        ...
        if self.shrink:
            self.shrinkage = RieszQuincunx(alpha)
    ...
    def forward(self, x):
        ...
        before_pool = x
        if self.shrink:
            before_pool = self.shrinkage(before_pool)
...
class UNet_RQ(nn.Module):
    def __init__(self, num_classes, segment, alpha, in_channels=3, depth=5, 
        ...
        for i in range(depth):
            ...
            #shrink = True if i < 2 else False
            #shrink = True

            down_conv = DownConv(ins, outs, alpha=self.alpha, pooling=pooling, batchnorm=batchnorm, dropout=dropout, shrink=shrink)
