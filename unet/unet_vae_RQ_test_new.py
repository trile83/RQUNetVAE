...
class DownConv(nn.Module):
    ...
    def forward(self, x):
        ...
        before_pool = self.shrinkRQ(before_pool)
...
class UNet_VAE_RQ_test(nn.Module):
    ...
    def forward(self, x):
        # Encoder
        x, bp_x = self.init_conv(x)
        x1, bp_x1 = self.down1(x)
        x2, bp_x2 = self.down2(x1)
        x3, bp_x3 = self.down3(x2)
        x4, bp_x4 = self.down4(x3)
        # Decoder
        ...
        x_up = self.up4(z, bp_x3)
        x_up = self.up3(x_up, bp_x2)
        x_up = self.up2(x_up, bp_x1)
        x_up = self.up1(x_up, bp_x)
