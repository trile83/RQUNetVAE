...
class RieszQuincunx(nn.Module):
    def __init__(self, alpha):
        ...
        if alpha == None:
            ...
            self.alpha = Parameter(torch.tensor(float(alpha)), requires_grad=True).cuda() # create a tensor out of alpha

        #self.alpha.data.uniform_(0, 1) ## check again
