...
class softShrink(nn.Module):
    ...
    def forward(self, x):
        ...
        if  x.detach().cpu().numpy().all() == 0:
            ...
            y = (x/(np.abs(x)+0.000000000000001))*np.maximum(np.abs(x)-alpha, 0)
            #y = (x/(torch.abs(x)+0.000000000000001))*torch.maximum(torch.abs(x)-self.alpha, torch.tensor(0.0))
...
class DownConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, alpha=0.0, pooling=True, dropout=False, shrink = False):
...
class UNet_VAE_Softshrink_All_trainable(nn.Module):
    def __init__(self, num_classes, alpha=None, in_channels=3, depth=5, 
        ...
        Arguments:
            ...
            up_mode: string, type of upconvolution. Choices: 'transpose'
                ...
                upsampling.
        """
        super(UNet_VAE_Softshrink_All_trainable, self).__init__()
    ...
    def forward(self, x):
        ...
        if  x.detach().cpu().numpy().all() == 0:
            print("tensor equal zero")
            mu = self.fc1(x_encoded)
            logvar = self.fc2(x_encoded)
            z = x
        else:
