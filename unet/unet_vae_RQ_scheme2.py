...
class UNet_VAE_RQ_scheme2(nn.Module):
    def __init__(self, num_classes, segment, alpha, in_channels=3, depth=5, 
        ...
        Arguments:
            ...
            up_mode: string, type of upconvolution. Choices: 'transpose'
                ...
                upsampling.
        """
        super(UNet_VAE_RQ_scheme2, self).__init__()
    ...
    def forward(self, x):
        ...
        for tau in range(0, Iter):    
                ...
                if i not in self.beta_D_I_dict.keys():
                    ...
                    for k in range(f_re.size(0)):
                        for l in range(f_re.size(1)):
                            ...
                            alpha = self.alpha
                            activation_method = "SoftShrink"
                            w_ikl = RieszWaveletTruncation(d_in, alpha, activation_method)
            ...
            # # Duy - edit to Torch: Inject standard Gaussian noise for Langevin dynamics here!
            # # epsilon = torch.normal(mu, std)
            # epsilon = np.random.randn(u.size())
            # u = u + np.sqrt(2)*epsilon
